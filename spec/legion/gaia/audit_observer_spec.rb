...
RSpec.describe Legion::Gaia::AuditObserver do
  ...
  describe '.process_event' do
    ...
    it 'exposes learned data for advisory' do
      described_class.process_event(event)
      learned = described_class.learned_data_for('user:matt')
    ...
    it 'does not raise on malformed event' do
      expect { described_class.process_event({}) }.not_to raise_error
      expect { described_class.process_event(nil) }.not_to raise_error
    end
  end

  describe '.user_preferences' do
    it 'returns empty hash for unknown identity' do
      expect(described_class.user_preferences('unknown')).to eq({})
    end
  end

  describe '.tool_patterns' do
    it 'returns empty hash when no events processed' do
      expect(described_class.tool_patterns).to eq({})
    end
  end
  ...
  describe '.reset!' do
    it 'clears all accumulated state' do
      ...
      expect(described_class.tool_patterns).to eq({})
      expect(described_class.user_preferences('user:matt')).to eq({})
    end
  end
end
